...
class Worker(WorkerBase):
    ...
    def init_device(self):
        if self.device_config.device_type == "cuda":
            ...
            # CMP 170HX: concurrent 5-worker CUDA context creation on this
            # platform sporadically fails with cudaErrorDevicesUnavailable /
            # cudaErrorMemoryAllocation — retry with backoff, and stagger the
            # workers so context creation does not race.
            import os as _os
            import time as _time

            _stagger = float(_os.environ.get("VLLM_WORKER_INIT_STAGGER_S", "0") or 0)
            if _stagger > 0:
                _time.sleep(self.local_rank * _stagger)
            _last = None
            for _attempt in range(5):
                try:
                    torch.accelerator.set_device_index(self.device)
                    _ = torch.zeros(8, device=self.device).sum().item()
                    _last = None
                    break
                except Exception as _e:  # noqa: BLE001
                    _last = _e
                    logger.warning(
                        "set_device_index(%s) attempt %d failed (%s); retrying",
                        self.device,
                        _attempt + 1,
                        _e,
                    )
