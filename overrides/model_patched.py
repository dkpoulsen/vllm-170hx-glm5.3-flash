...
class Glm5NextDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if _os.environ.get("VLLM_LAYER_DIAG") == "1":
            _shp = lambda t: None if t is None else (tuple(t.shape), str(t.dtype))
            logger.warning(
                "DIAG layer=%s mhc=%s in hs=%s res=%s post=%s comb=%s",
                self.layer_idx,
                self.mhc,
                _shp(hidden_states),
                _shp(residual),
        ...
        if post is None:
            if residual is not None and residual.dim() == 3:
                # PP rank boundary: the incoming residual is already
                # hc-expanded by the previous rank's first layer.
                x = residual
            else:
...
class Glm5NextModel(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # PP unlock: hidden_states/residual are the only tensors crossing a
        # stage boundary (see forward); the deferred mHC post/comb state is
        # rank-local and the next rank's first layer falls back to standalone
        # hc_pre, which is mathematically equivalent. The residual crosses in
        # its hc-expanded form [tokens, mhc_num_residual_streams, hidden].
        _hidden = config.hidden_size
        _n_streams = getattr(config, "mhc_num_residual_streams", None)
        _is_mhc = bool(getattr(config, "mhc", False)) and _n_streams is not None

        def _make_empty_intermediate_tensors(batch_size, dtype, device):
            res_shape = (
                (batch_size, _n_streams, _hidden) if _is_mhc else (batch_size, _hidden)
            )
            return IntermediateTensors(
                {
                    "hidden_states": torch.zeros(
                        (batch_size, _hidden), dtype=dtype, device=device
                    ),
                    "residual": torch.zeros(res_shape, dtype=dtype, device=device),
                }
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if _os.environ.get("VLLM_LAYER_DIAG") == "1":
            _shp = lambda t: None if t is None else tuple(t.shape)
            logger.warning(
                "DIAG modelfwd rank_start=%s tokens=%s hs=%s res=%s",
        ...
        try:
            ...
            logger.error(
                "DIAG layer loop failed at layer=%s: hs=%s res=%s post=%s comb=%s : %s",
                layer.layer_idx,
                None if hidden_states is None else tuple(hidden_states.shape),
                None if residual is None else tuple(residual.shape),
                None if post is None else tuple(post.shape),
                None if comb is None else tuple(comb.shape),
                _e,
            )
            raise
        ...
        if not get_pp_group().is_last_rank:
            ...
            # layer; they are not propagated across the stage boundary — the
            # receiving rank's first layer falls back to standalone pre
            # (equivalent math, one less fused kernel at the seam).
            return IntermediateTensors(
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # CMP 170HX: serialize the per-rank weight-loading loop (H2D DMA
        # burst) across PP workers — concurrent 5-way PCIe x4 Gen2 load
        # traffic triggers link retrains (AER Data-Link-Layer timeouts) that
        # surface as async illegal accesses later. One rank loads at a time.
        import fcntl as _fcntl
        import os as _os

        _lock_path = _os.environ.get("VLLM_MARLIN_LOCK", "")
        _lf = open(_lock_path, "a") if _lock_path else None
        if _lf is not None:
        ...
        try:
            for args in weights:
                ...
                for param_name, weight_name, shard_id in stacked_params_mapping:
                    ...
                    # QKV fusion / indexer fusion: skip if fused module doesn't
                    # exist in model (dense mode has no wk_weights_proj target)
                    if (
                    ...
                    for idx, (
                        ...
                        if (
                            ...
                            continue
                        # Dense-mode (index_topk=null) skip: the checkpoint still
                        # carries the DSA indexer weights; the module is absent.
...
class Glm5NextForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # PP unlock: delegate to the text model (Glm5NextModel now provides
        # make_empty_intermediate_tensors), mirroring Glm4vForConditionalGeneration.
...
def _try_load_fp8_indexer_wk(name, tensor, buf, params_dict, loaded_params):
    ...
    # Dense-mode (index_topk=null): the fused indexer target is absent —
    # consume the buffered pieces and skip.
