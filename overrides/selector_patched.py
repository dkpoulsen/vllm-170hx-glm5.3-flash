...
def _get_mla_prefill_backend_priorities(
    ...
    elif device_capability.major == 9:  # Hopper
        return [
            MLAPrefillBackendEnum.FLASH_ATTN,
        ]
    else:  # sm_80 and older (CMP 170HX patch): no FA3/FA4/FlashInfer MLA —
        # prefer the ragged Triton backend registered as CUSTOM, FA2 fallback.
        return [
...
# CMP 170HX patch: import the sm_80 ragged Triton MLA prefill backend so it is
# registered as MLAPrefillBackendEnum.CUSTOM before any selection happens.
from vllm.v1.attention.backends.mla.prefill.triton_mla_prefill_sm80 import (  # noqa: E402, F401
    TritonMlaPrefillSm80,
)
