            ...
            with safe_open(st_file, framework="pt") as f:
                for name in f.keys():  # noqa: SIM118
                    ...
                    try:
                        ...
                        # CMP 170HX diag/fallback: under the full engine context
                        # get_tensor sporadically fails with
                        # "could not determine the shape of object type
                        # 'torch.storage.UntypedStorage'" (torch._refs dispatch
                        # intercepting safetensors' pybind tensor construction).
                        # Re-read via the numpy framework and wrap — bypasses
                        # the torch pybind path entirely.
                        logger.warning_once(
                            "get_tensor failed for %s (%s); retrying via numpy",
                            name,
                            e,
                        )
                        with safe_open(st_file, framework="numpy") as f2:
                            arr = f2.get_tensor(name)
